...
def make_fooling_image(X, target_y, model):
    ...
    while True:
        ...
        if torch.argmax(scores, 1) == target_y:
            break
        
        print(f"Iteration: {iteration}  dif: {(torch.max(scores, 1)[0] - scores[0,target_y]).item()}")
        iteration += 1
        ...
        with torch.no_grad():
            X_fooling += learning_rate * grad / torch.sum(grad * grad)
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    img.grad = None
    f = score - l2_reg * torch.sum(img * img)
    f.backward()
    grad = img.grad
    with torch.no_grad():
