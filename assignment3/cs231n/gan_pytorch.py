...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_real, torch.ones(N).type_as(logits_real)) \
            + bce_loss(logits_fake, torch.zeros(N).type_as(logits_fake))
...
def generator_loss(logits_fake):
    ...
    loss = bce_loss(logits_fake, torch.ones(logits_fake.shape[0]).type_as(logits_fake))
