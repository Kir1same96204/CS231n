...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    f = np.dot(prev_h, Wh) + np.dot(x, Wx) + b
    next_h = np.tanh(f)

    cache = (x, prev_h, Wx, Wh, f)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, f = cache
    df = dnext_h / (np.cosh(f)**2)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (next_c, i, f, o, g, a[:, 3*H:4*H], Wx, Wh, x, prev_h, prev_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    N, H = dnext_c.shape
    next_c, i, f, o, g, ag, Wx, Wh, x, prev_h, prev_c = cache
    do = dnext_h * np.tanh(next_c)
    dnext_c += dnext_h * o / (np.cosh(next_c) ** 2)
    dprev_c = dnext_c * f
    di = dnext_c * g
    df = prev_c * dnext_c
    dg = dnext_c * i
    da = np.zeros((N, 4*H))
    da[:, 0:H] = di * i * (np.ones_like(i) - i)
    da[:, H:2*H] = df * f * (np.ones_like(f) - f)
    da[:, 2*H:3*H] = do * o * (np.ones_like(o) - o)
    da[:, 3*H:4*H] = dg / (np.cosh(ag) ** 2)
    dx = da @ Wx.T
    dprev_h = da @ Wh.T
    dWx = x.T @ da
    dWh = prev_h.T @ da
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = {}
    for t in range(T):
        next_h, next_c, cache[t] = lstm_step_forward(x[:,t,:], next_h, next_c, Wx, Wh, b)
        h[:, t, :] = next_h
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dprev_h += dh[:, t, :]
        dx_, dprev_h, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dprev_h, dprev_c, cache[t])
        dx[:, t, :] = dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
