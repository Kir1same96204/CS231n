...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss -= 2*sim(z_k, z_k_N) / tau
        div = torch.exp(torch.tensor([[sim(z_i, z_j) / tau for j, z_j in enumerate(out) if i != j] \
                                        for i, z_i in [(k, z_k), (k+N, z_k_N)]]
            )
        )
        total_loss += torch.log(torch.sum(div, dim=1)).sum()
...
def sim_positive_pairs(out_left, out_right):
    ...
    left = out_left / torch.linalg.norm(out_left, dim=1, keepdims=True)
    right = out_right / torch.linalg.norm(out_right, dim=1, keepdims=True)
    pos_pairs = torch.sum(left * right, dim=1, keepdims=True)
